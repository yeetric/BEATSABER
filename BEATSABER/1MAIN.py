...
with open("abcNotation.txt") as f: #open abc notation file
    content = f.readlines() #read lines
abcNotation = str(content[0]) #get first line
abcNotation = abcNotation.replace(" ", "") #remove spaces
abcNotation = abcNotation.replace("-", "") #remove dashes
abcNotation = abcNotation.replace("1", "") #remove numbers
abcNotation = abcNotation.replace("2", "")
abcNotation = abcNotation.replace("3", "")
abcNotation = abcNotation.replace("4", "")
abcNotation = abcNotation.replace("5", "")
abcNotation = abcNotation.replace("6", "")
abcNotation = abcNotation.replace("7", "")
abcNotation = abcNotation.replace("8", "")
abcNotation = abcNotation.replace("9", "")
abcNotation = abcNotation.replace("0", "")
abcNotation = abcNotation.replace(".", "") #remove dots
notes=[] #empty list for notes
for note in abcNotation:
    notes.append(note) #add notes to list
...
def main():
    ...
    while True:
        ...
        while not BLUEtouching or not REDtouching:  # if touching is true, the box will be reset
                ...
                for id, lm in enumerate(results.pose_landmarks.landmark):
                    ...
                    if id == 20 or id == 19:  # arms
                        ...
                        if id == 20:
                            ...
                            secondStickRightHandX, secondStickRightHandY = (
                                (cx) + 3 * (int(cx - rightWristX)), cy + 3 * (int(cy - rightWristY)))

                            cv2.line(img, (cx, cy), (int(secondStickRightHandX), int(secondStickRightHandY)),
                                     (255, 0, 0), 10)  # draws outline of stick
                            cv2.line(img, (cx, cy), (int(secondStickRightHandX), int(secondStickRightHandY)),
                                     (255, 255, 255), 3)  # draws center glow line

                            if BLUElength < boxSize + 20:  # tests if touching (around area)
                                mixer.music.load(f"{notes[whichNote]}.mp3")  # plays sound. change file name to change sound
                                mixer.music.play()
                            ...
                            secondStickLeftHandX, secondStickLeftHandY = (
                                (cx) + 3 * (int(cx - leftWristX)), cy + 3 * (int(cy - leftWristY)))
                            print(secondStickLeftHandX, secondStickLeftHandY)
                            cv2.line(img, (cx, cy), (int(secondStickLeftHandX), int(secondStickLeftHandY)), (0, 0, 255),
                                     10)
                            cv2.line(img, (cx, cy), (int(secondStickLeftHandX), int(secondStickLeftHandY)),
                                     (255, 255, 255), 3)
                            REDlength = math.hypot(cx - REDmid[0], cy - REDmid[1])

                            if REDlength < boxSize + 20: # tests if touching (around area)
                                mixer.music.load(
                                    f"{notes[whichNote]}.mp3")  # plays sound. change file name to change sound
                                mixer.music.play()
                                whichNote += 1  # increments which note to play
                                score += 1  # increment score
                                REDtouching = True
            ...
            cv2.putText(img, str(int(score)), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255),
                        3)  # score display. must be after flip or else text would be flipped
